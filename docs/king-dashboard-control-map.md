...
| Field agents | `/admin/field-agents`, `/admin/field-agent-payouts`, `/admin/field-agent-training`, `/admin/field-agent-notices`, `/admin/contracts` | `/api/admin/field-agents/provision`, `/api/admin/field-agents/:id/documents`, `/api/admin/field-agents/:id/payment`, `/api/admin/field-agents/broadcast`, field-agent dashboard APIs | Field Agent Control Centre: auto-generated production-range ID/PIN setup, ID document upload, signed contract upload, directory, review details, contact/delete/pause controls, accepted/pending/rejected counts, Friday payout review at USh 5,000 per approved listing, payment recording with receipt/proof, territory broadcasts, dashboard banner, resources | hashed PIN only, onboarding EmailLog/WhatsAppMessageLog/NotificationLog, document/payment logs, broadcast logs, AdminAuditLog, activity logs | Working for admin-created agents |
