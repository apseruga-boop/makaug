...
| Header | Discover AI Chatbot | `#nav-ai` | `/discover-ai-chatbot` | Public route | Public route | `page_view` | click probe | working | Chatbot route opens connected AI task panel plus WhatsApp support. |
| Header | About Us | `#nav-about` | `/about` | Public About route | Public About route | `page_view` | click probe | working | Task 12 replaces primary Fraud nav with About Us. Anti-Fraud remains in footer/support. |
...
| List property | List via WhatsApp | `#lp-whatsapp-option-btn`, `#lp-wa-link` | Opens official WhatsApp with listing type/title/location context | Public WhatsApp flow | Same | `list_property_whatsapp_clicked` | click probe + go-live tests | working | Message strips undefined/null and includes context when entered. |
