...
function renderAdvertiseSelfServeRouteContent() {
  return `
  <main id="page-advertise" class="page active" data-public-route="/advertise">
    <span class="sr-only" data-release-marker="advertising-selfserve-v1-20260713">advertising-selfserve-v1-20260713</span>
    <section class="bg-[#0f3d2e] py-10 text-white">
      <div class="max-w-5xl mx-auto px-4">
        <p class="text-green-100 text-sm font-bold uppercase tracking-wide">Self-serve advertising</p>
        <h1 class="text-4xl font-black serif mt-2">Advertise on makaug in minutes</h1>
        <p class="text-green-50 mt-3 max-w-3xl">Pick a placement, choose duration, generate an on-brand ad, preview it, then use Flutterwave hosted checkout for mobile money, card, or PayPal where available. King approves before it goes live.</p>
        <div class="flex flex-wrap gap-2 mt-5">
          <a href="#advertising-selfserve-form" class="inline-flex rounded-lg bg-white px-4 py-2 text-sm font-black text-[#15603f]">Start advertising</a>
          <a href="https://wa.me/256760112587?text=Hello%20makaug,%20I%20need%20help%20booking%20an%20advertising%20campaign." target="_blank" rel="noopener noreferrer" class="inline-flex rounded-lg border border-white/40 px-4 py-2 text-sm font-black text-white" data-public-whatsapp-link data-whatsapp-context="advertise">WhatsApp help</a>
        </div>
      </div>
    </section>
    <section class="max-w-6xl mx-auto px-4 py-10">
      <form id="advertising-selfserve-form" onsubmit="submitAdvertisingSelfServeCampaign(event)" class="rounded-3xl border border-green-100 bg-white p-4 md:p-5">
        <div class="flex flex-wrap items-center justify-between gap-3 mb-4">
          <div><h2 class="text-xl font-black text-gray-900">Campaign builder</h2><p class="text-xs text-gray-500">5 steps: placement, duration, creative, preview, pay.</p></div>
        ...
        <div class="grid lg:grid-cols-[1.05fr_0.95fr] gap-5">
          <div class="space-y-4">
            <section data-ad-ss-step="1" class="space-y-3"><h3 class="font-black text-gray-900">1. Choose placement</h3><div id="advertising-ss-placement-grid" class="grid md:grid-cols-3 gap-3"><div class="rounded-2xl border border-gray-200 bg-gray-50 p-4 text-sm text-gray-500">Loading placements...</div></div></section>
            <section data-ad-ss-step="2" class="hidden space-y-3"><h3 class="font-black text-gray-900">2. Duration</h3><div class="grid sm:grid-cols-4 gap-2"><button type="button" onclick="setAdvertisingDuration(7)" data-ad-duration="7" class="rounded-xl border border-green-200 bg-green-50 px-3 py-3 text-sm font-black text-green-800">1 week</button><button type="button" onclick="setAdvertisingDuration(14)" data-ad-duration="14" class="rounded-xl border border-gray-200 px-3 py-3 text-sm font-black text-gray-700">2 weeks</button><button type="button" onclick="setAdvertisingDuration(28)" data-ad-duration="28" class="rounded-xl border border-gray-200 px-3 py-3 text-sm font-black text-gray-700">4 weeks</button><label class="rounded-xl border border-gray-200 px-3 py-2 text-xs text-gray-500">Custom days<input id="advertising-ss-duration" type="number" min="3" value="7" oninput="setAdvertisingDuration(this.value, true)" class="mt-1 w-full text-sm font-black text-gray-900 outline-none"></label></div><label class="block text-sm font-bold text-gray-800">Start date<input id="advertising-ss-start-date" type="date" onchange="renderAdvertisingSelfServePreview()" class="mt-2 w-full rounded-xl border border-gray-200 px-4 py-3 text-sm"></label></section>
            <section data-ad-ss-step="3" class="hidden space-y-3"><h3 class="font-black text-gray-900">3. Creative</h3><textarea id="advertising-ss-brief" rows="3" oninput="syncAdvertisingCreativeFromInputs()" class="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm" placeholder="What are you advertising?"></textarea><button type="button" onclick="generateAdvertisingAiDraft()" class="rounded-xl bg-[#15603f] px-4 py-2 text-sm font-black text-white">Generate my ad with AI</button><div class="grid md:grid-cols-2 gap-3"><input id="advertising-ss-headline" maxlength="64" oninput="syncAdvertisingCreativeFromInputs()" class="rounded-xl border border-gray-200 px-4 py-3 text-sm" placeholder="Headline"><input id="advertising-ss-cta" maxlength="24" oninput="syncAdvertisingCreativeFromInputs()" class="rounded-xl border border-gray-200 px-4 py-3 text-sm" placeholder="CTA label"></div><input id="advertising-ss-line" maxlength="120" oninput="syncAdvertisingCreativeFromInputs()" class="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm" placeholder="Supporting line"><input id="advertising-ss-url" type="url" oninput="syncAdvertisingCreativeFromInputs()" class="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm" placeholder="Destination URL"><div class="grid md:grid-cols-2 gap-3"><select id="advertising-ss-image-source" onchange="syncAdvertisingCreativeFromInputs()" class="rounded-xl border border-gray-200 px-4 py-3 text-sm bg-white"><option value="ai">AI image</option><option value="upload">Upload / own photo</option><option value="url">Image URL</option></select><input id="advertising-ss-image-url" oninput="syncAdvertisingCreativeFromInputs()" class="rounded-xl border border-gray-200 px-4 py-3 text-sm" placeholder="Image URL"></div><input id="advertising-ss-image-file" type="file" accept="image/png,image/jpeg,image/webp" onchange="handleAdvertisingImageUploadPreview(event)" class="block w-full rounded-xl border border-gray-200 px-4 py-3 text-sm"><p id="advertising-ss-validation" class="text-xs text-gray-500">Creative checks appear here.</p></section>
            <section data-ad-ss-step="4" class="hidden space-y-3"><h3 class="font-black text-gray-900">4. Confirm preview</h3><div class="grid sm:grid-cols-3 gap-2"><button type="button" onclick="setAdvertisingPreviewDevice('mobile')" data-ad-device="mobile" class="rounded-xl bg-green-50 border border-green-200 px-3 py-2 text-sm font-bold text-green-800">Mobile</button><button type="button" onclick="setAdvertisingPreviewDevice('desktop')" data-ad-device="desktop" class="rounded-xl border border-gray-200 px-3 py-2 text-sm font-bold text-gray-700">Desktop</button><select id="advertising-ss-language" onchange="setAdvertisingPreviewLanguage(this.value)" class="rounded-xl border border-gray-200 px-3 py-2 text-sm bg-white"><option value="en">English</option><option value="lg">Luganda</option><option value="sw">Kiswahili</option><option value="ach">Acholi</option><option value="nyn">Runyankole</option><option value="rn">Rukiga</option><option value="lus">Lusoga</option><option value="am">Amharic</option><option value="ar">Arabic</option></select></div><p id="advertising-ss-translation-note" class="text-xs text-gray-500">Switch language to auto-translate the ad copy.</p></section>
            <section data-ad-ss-step="5" class="hidden space-y-3"><h3 class="font-black text-gray-900">5. Pay</h3><div class="grid md:grid-cols-2 gap-3"><input id="advertising-ss-name" class="rounded-xl border border-gray-200 px-4 py-3 text-sm" placeholder="Your name" required><input id="advertising-ss-business" class="rounded-xl border border-gray-200 px-4 py-3 text-sm" placeholder="Business / brand"><input id="advertising-ss-email" type="email" class="rounded-xl border border-gray-200 px-4 py-3 text-sm" placeholder="Email"><input id="advertising-ss-phone" class="rounded-xl border border-gray-200 px-4 py-3 text-sm" placeholder="WhatsApp / phone"></div><div class="grid sm:grid-cols-3 gap-2"><label class="rounded-xl border border-green-200 bg-green-50 px-3 py-3 text-sm font-bold"><input type="radio" name="advertising-ss-payment" value="paypal" checked> PayPal via Flutterwave</label><label class="rounded-xl border border-gray-200 px-3 py-3 text-sm font-bold"><input type="radio" name="advertising-ss-payment" value="mobile_money"> MTN/Airtel Mobile Money</label><label class="rounded-xl border border-gray-200 px-3 py-3 text-sm font-bold"><input type="radio" name="advertising-ss-payment" value="card"> Card</label></div><p class="text-xs text-gray-500">Card, PayPal, and mobile-money details stay inside Flutterwave hosted checkout.</p></section>
            <div class="flex items-center justify-between gap-3 pt-2"><button type="button" onclick="setAdvertiseSelfServeStep((advertisingSelfServeState.step || 1) - 1)" class="rounded-xl border border-gray-200 px-4 py-2 text-sm font-black text-gray-700">Back</button><div class="flex gap-2"><button type="button" onclick="setAdvertiseSelfServeStep((advertisingSelfServeState.step || 1) + 1)" class="rounded-xl bg-[#15603f] px-4 py-2 text-sm font-black text-white">Next</button><button type="submit" class="rounded-xl bg-[#0f3d2e] px-4 py-2 text-sm font-black text-white">Create checkout</button></div></div><div id="advertising-ss-status" class="hidden rounded-xl border border-green-200 bg-green-50 p-3 text-sm text-green-900"></div>
          </div>
          <aside class="space-y-3"><div id="advertising-ss-quote" class="rounded-2xl border border-green-100 bg-[#f6f9f7] p-4 text-sm">Choose a placement to see the live quote.</div><div id="advertising-ss-live-preview" class="rounded-3xl border border-green-100 bg-white p-4"></div><div class="rounded-2xl border border-gray-200 bg-gray-50 p-3 text-xs text-gray-600">King sees payment status, auto-checks, destination URL, language set, and creative preview before approving.</div></aside>
        </div>
      </form>
    </section>
  </main>`;
...
function renderSyntheticRouteContent(pathname = '/') {
  ...
  if (!content) return '';
  if (pathName === '/advertise') return renderAdvertiseSelfServeRouteContent();
