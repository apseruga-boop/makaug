    ...
    `INSERT INTO source_drip_state (
     ...
     ON CONFLICT (drip_key) DO UPDATE
       SET source_count = EXCLUDED.source_count,
           batch_size = CASE
             WHEN source_drip_state.search_mode = 'all' THEN LEAST(GREATEST(source_drip_state.batch_size, 25), 25)
...
function summarizeSweepResult(result = {}) {
  ...
  return {
    ...
    foreign_rejected_count: Number(importResult.foreign_rejected_count || 0),
...
async function logDripNotification(db, summary = {}, status = 'logged', failureReason = '') {
  await logNotification(db, {
    ...
    payloadSummary: {
      ...
      foreign_rejected_count: summary.foreign_rejected_count,
