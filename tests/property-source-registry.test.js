...
test('public pages explain the search-engine model and expose found-online source metadata', () => {
  ...
  assert(read('routes/properties.js').includes('function publicContactPhoneForRow'), 'public property API should derive a safe public phone for found-online contact buttons');
  assert(read('routes/properties.js').includes('public_contact_phone: publicContactPhone || null'), 'public property API should expose safe public contact phones separately from redacted lister_phone');
  assert(read('routes/properties.js').includes('publicUgandaContactPhoneFromText'), 'public property API should recover valid Uganda phones from source captions/text when stored in dashboard evidence');
