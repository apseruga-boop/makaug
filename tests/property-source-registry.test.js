...
test('source registry service defines a multi-platform Uganda property source database', () => {
  ...
  assert(service.includes('first published from 1 January 2022 through today'), 'source records should scan from the 2022 found-online window start');
  assert(service.includes('target_source_year: TARGET_SOURCE_YEAR'), 'source records should flag 2022 as the active target source year');
...
test('daily source sweep is scriptable and keeps King queue guardrails', () => {
  ...
  assert(dailySweepScript.includes('source_post_window_start: LAUNCH_SOURCE_POST_WINDOW_START'), 'daily sweep should expose the 1 January 2022 found-online intake start');
  assert(dailySweepScript.includes('target_source_year: 2022'), 'daily sweep should prioritise 2022+ source posts');
