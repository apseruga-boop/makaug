...
function run() {
  ...
  assert.strictEqual(Object.keys(LANGUAGE_REGISTRY).length, 9, 'language registry should expose nine supported languages');
  assert(LANGUAGE_REGISTRY.am && LANGUAGE_REGISTRY.am.displayName === 'Amharic', 'language registry should define Amharic');
  assert(LANGUAGE_REGISTRY.ar && LANGUAGE_REGISTRY.ar.displayName === 'Arabic', 'language registry should define Arabic');
  assert.strictEqual(LANGUAGE_REGISTRY.ar.direction, 'rtl', 'Arabic should be marked as right-to-left');
  assert.strictEqual(toLegacyLanguageCode('8'), 'am', 'WhatsApp language option 8 should select Amharic');
  assert.strictEqual(toLegacyLanguageCode('9'), 'ar', 'WhatsApp language option 9 should select Arabic');
  assert.strictEqual(toLegacyLanguageCode('amharic'), 'am', 'Amharic alias should resolve to legacy am code');
  assert.strictEqual(toLegacyLanguageCode('arabic'), 'ar', 'Arabic alias should resolve to legacy ar code');
  assert.strictEqual(toLegacyLanguageCode('العربية'), 'ar', 'Arabic native alias should resolve to legacy ar code');
  assert.strictEqual(languageDisplayName('am'), 'Amharic', 'language display name should include Amharic');
  assert.strictEqual(languageDisplayName('ar'), 'Arabic', 'language display name should include Arabic');
  assert.strictEqual(SUPPORTED_AI_LANGUAGES.am, 'Amharic', 'AI language list should include Amharic');
  assert.strictEqual(SUPPORTED_AI_LANGUAGES.ar, 'Arabic', 'AI language list should include Arabic');
  ...
  assert(sourceHtml.includes('value="ar">Arabic</option>'), 'public language dropdowns should include Arabic');
  assert(sourceHtml.includes('amharic-i18n-20260529'), 'public HTML should bump the app asset version when Amharic UI changes');
  assert(sourceHtml.includes('arabic-i18n-20260606'), 'public HTML should bump the app asset version when Arabic UI changes');
  assert(frontendSource.includes('I18N_UI.am'), 'frontend UI translations should include Amharic');
  assert(frontendSource.includes('I18N_UI.ar'), 'frontend UI translations should include Arabic');
  ...
  assert(frontendSource.includes('CONTENT_I18N.ar'), 'content translations should include Arabic');
  assert(frontendSource.includes('PROPERTY_UI_I18N.am'), 'property page translations should include Amharic');
  assert(frontendSource.includes('PROPERTY_UI_I18N.ar'), 'property page translations should include Arabic');
  assert(frontendSource.includes('LISTING_LABEL_I18N_SUPPLEMENTAL.am'), 'listing labels should include Amharic');
  assert(frontendSource.includes('LISTING_LABEL_I18N_SUPPLEMENTAL.ar'), 'listing labels should include Arabic');
  assert(frontendSource.includes('document.documentElement.dir = currentLang === "ar" ? "rtl" : "ltr"'), 'frontend should switch document direction for Arabic');
  assert(whatsappRoutes.includes("'8': 'am'"), 'WhatsApp language menu should route option 8 to Amharic');
  assert(whatsappRoutes.includes("'9': 'ar'"), 'WhatsApp language menu should route option 9 to Arabic');
  ...
  assert(frontendSource.includes('normalizeAdminFieldAgentPhone') && frontendSource.includes('Short local test numbers are not saved'), 'admin field-agent setup should normalize full phone numbers and explain why short numbers are not saved');
  assert(frontendSource.includes('adminHighlightFieldAgentRow') && frontendSource.includes('Directory refreshed below'), 'admin field-agent save should refresh and highlight the newly saved directory row');
  ...
  for (const fieldAgentKpiRoute of ['/admin/field-agent-active', '/admin/field-agent-pending', '/admin/field-agent-accepted', '/admin/field-agent-rejected', '/admin/field-agent-payouts']) {
    assert(frontendSource.includes(`data-king-shortcut="${fieldAgentKpiRoute}"`), `King shortcuts should open Field Agent KPI route: ${fieldAgentKpiRoute}`);
  }
  for (const fieldAgentKpiKind of ['fieldAgentKpi: "active"', 'fieldAgentKpi: "pending"', 'fieldAgentKpi: "approved"', 'fieldAgentKpi: "rejected"', 'fieldAgentKpi: "payouts"']) {
    assert(frontendSource.includes(fieldAgentKpiKind), `admin route map should deep-link Field Agent drill-down: ${fieldAgentKpiKind}`);
  }
  ...
  assert(frontendSource.includes('adminLoadFieldAgentPayouts') && frontendSource.includes('/api/admin/field-agents/payouts?period='), 'Friday payday drill-down should load a backend payout ledger with selectable periods');
  assert(frontendSource.includes('adminSetFieldAgentPayoutPeriod') && frontendSource.includes('Current due ledger') && frontendSource.includes('Previous Friday'), 'Friday payday drill-down should allow current and historical payout windows');
  ...
  assert(adminRoutes.includes('normalizeFieldAgentContactPhone') && adminRoutes.includes('Enter a full phone number with country code'), 'admin API should normalize field-agent contact phones and return clear validation errors');
  ...
  assert(adminRoutes.includes("router.get('/field-agents/payouts'"), 'admin API should expose a Field Agent payout ledger endpoint');
  assert(adminRoutes.includes('fieldAgentPayoutWindow') && adminRoutes.includes('normalizeFieldAgentPayoutPeriod'), 'admin API should normalize payout periods for current and historical Friday drill-downs');
  for (const payoutPeriod of ['current_due', 'this_friday', 'previous_friday', 'last_7_days']) {
    assert(adminRoutes.includes(payoutPeriod), `admin payout ledger should support period: ${payoutPeriod}`);
  }
  assert(adminRoutes.includes('period_start') && adminRoutes.includes('period_end') && adminRoutes.includes('pay_by_date'), 'admin payout ledger should explain the week/window and pay-by date');
  assert(adminRoutes.includes('total_due_ugx') && adminRoutes.includes('due_count'), 'admin payout ledger should return due totals for King drill-down cards');
  ...
  assert(adminRoutes.includes('field_agent_registered_email') && adminRoutes.includes('field_agent_registered_whatsapp'), 'field-agent provisioning should log email and WhatsApp onboarding notifications');
  assert(adminRoutes.includes('emailProviderConfigured()'), 'field-agent PIN email should not be mocked into logs when email providers are missing');
  assert(adminRoutes.includes('Please check your email for your private 4-digit PIN'), 'field-agent WhatsApp onboarding should not include the PIN');
  ...
  assert(frontendSource.includes('id="account-access-create-broker-id-step"') && frontendSource.includes('Broker ID details') && frontendSource.includes('broker_signup_without_contact_otp'), 'broker account drawer should require National ID details while skipping unavailable contact OTP');
  ...
  assert(frontendSource.includes('id="broker-leads-panel"') && frontendSource.includes('Shareable broker card') && frontendSource.includes('id="broker-boost-panel"') && frontendSource.includes('broker-dashboard-whatsapp-share-card'), 'broker dashboard should include lead centre, social broker card sharing, and boost planning surfaces');
  ...
  assert(frontendSource.includes('function brokerDashboardListingBelongsToBroker') && frontendSource.includes('Boost after approval') && frontendSource.includes('BROKER_BOOST_FORMATS'), 'broker dashboard should keep listings tied to the signed-in broker and only boost approved live listings');
  assert(frontendSource.includes("shareBrokerCard('facebook')") && frontendSource.includes("shareBrokerCard('linkedin')") && frontendSource.includes("shareBrokerCard('x')") && frontendSource.includes("shareBrokerCard('link')"), 'broker dashboard share card should support social sharing and direct link copy');
  assert(frontendSource.includes('function findBrokerByIdIncludingLoadedProfile') && frontendSource.includes('existing?.remote_profile_loaded && id && !nextIds.has(id)') && frontendSource.includes('const broker = findBrokerByIdIncludingLoadedProfile(id)'), 'public broker profile share should keep and resolve direct-loaded pending broker cards even after approved-directory refresh');
  assert(frontendSource.includes('function showCopyFallbackLink') && frontendSource.includes('copy-fallback-panel') && frontendSource.includes('Copy the broker profile link from the box.'), 'broker profile share should show a selectable fallback link when browser clipboard copy is blocked');
  assert(frontendSource.includes('wireBrokerDashboardTabs') && frontendSource.includes('broker_dashboard_tab'), 'broker dashboard tabs should update the active tab and target panel');
  assert(agentsRoutes.includes("router.get('/me'") && agentsRoutes.includes("router.patch('/me'") && agentsRoutes.includes('can_skip_listing_identity_upload') && agentsRoutes.includes('lead_enquiries'), 'broker dashboard API should expose protected profile/listing data, lead counts, and broker listing capabilities');
  assert(agentsRoutes.includes("COALESCE(p.extra_fields, '{}'::jsonb)->>'broker_agent_id'") && !agentsRoutes.includes("LOWER(COALESCE(p.lister_email, '')) = LOWER($2)") && !agentsRoutes.includes("regexp_replace(COALESCE(p.lister_phone, ''), '\\\\D', '', 'g') = $3"), 'broker dashboard API must not pull unrelated listings by shared email or phone fallback');
  assert(agentsRoutes.includes("LOWER(COALESCE(a.status, 'pending')) NOT IN ('rejected', 'declined', 'suspended', 'deleted', 'removed', 'blocked')"), 'public broker profile route should allow safe self-registered pending profiles without exposing rejected or social-source agents');
  const publicAgentListRoute = agentsRoutes.slice(agentsRoutes.indexOf("router.get('/',"), agentsRoutes.indexOf("router.get('/:id'"));
  const publicAgentProfileRoute = agentsRoutes.slice(agentsRoutes.indexOf("router.get('/:id'"), agentsRoutes.indexOf("router.post('/register'"));
  const anonymousAgentRoutes = `${publicAgentListRoute}\n${publicAgentProfileRoute}`;
  assert(!/SELECT\s+a\.\*/.test(anonymousAgentRoutes) && !anonymousAgentRoutes.includes('a.identity_document_url') && !anonymousAgentRoutes.includes('a.privacy_consent_accepted') && !anonymousAgentRoutes.includes('a.data_retention_notice_accepted'), 'anonymous broker directory/profile APIs must not expose raw agent rows, National ID uploads, or private consent metadata');
  assert(propertiesRoutes.includes('brokerCanSkipOwnerIdentity') && propertiesRoutes.includes('agent_id = $2') && propertiesRoutes.includes("lister_type = 'agent'"), 'property submission API should link signed-in broker listings to agent_id while keeping review pending');
  assert(whatsappRoutes.includes('findBrokerAgentByContact') && whatsappRoutes.includes('whatsapp_broker_fast_track'), 'WhatsApp listing flow should match broker email/phone and attach listings to broker profiles');
  ...
  assert(frontendSource.includes('admin_api_key_fallback'), 'King dashboard deep link should render the API-key reconnect gate for signed-in non-admin sessions');
  assert(serverSource.includes('shouldServeAdminShellForApiKeyFallback') && serverSource.includes('X-makaug-Admin-Api-Key-Fallback'), 'server should serve the King shell for authenticated API-key fallback without granting moderator role access');
  ...
  assert(adminRoutes.includes('base.durationMs') && adminRoutes.includes('base.attemptedProviders'), 'admin email provider proof should expose send timing and provider attempts');
  ...
  assert(emailServiceSource.includes('emailProviderOrder') && emailServiceSource.includes("order.push('resend')"), 'email service should prefer the verified Resend provider before slower legacy SMTP fallback');
  assert(emailServiceSource.includes('attemptedProviders') && emailServiceSource.includes('SMTP_CONNECTION_TIMEOUT_MS'), 'email service should expose provider attempts and cap slow SMTP fallback delays');
  ...
  assert(whatsappRoutes.includes('WHATSAPP_WEB_BRIDGE_TEXT_INBOUND_DEDUPE_SECONDS || 180'), 'WhatsApp text fingerprints must outlive slow browser rescans that otherwise create duplicate replies');
  ...
  assert(whatsappWebCopilotScript.includes("WHATSAPP_WEB_COPILOT_TRUST_SEND_ON_COMPOSER_CLEAR || 'false'"), 'WhatsApp Web copilot must require a visible outgoing bubble by default, including on hosted headless WhatsApp Web');
