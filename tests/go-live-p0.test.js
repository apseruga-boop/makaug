...
function run() {
  ...
  assert(frontendSource.includes('id="broker-settings-panel"') && frontendSource.includes('saveBrokerProfileSettings') && frontendSource.includes('Public broker card preview'), 'broker dashboard should expose editable broker profile settings and a public card preview');
  assert(agentsRoutes.includes("router.get('/me'") && agentsRoutes.includes("router.patch('/me'") && agentsRoutes.includes('can_skip_listing_identity_upload'), 'broker dashboard API should expose protected profile/listing data and broker listing capabilities');
