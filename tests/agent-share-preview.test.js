...
const brandCardPath = path.join(root, 'assets', 'marketing', 'francis-isabirye-agent-share-v4.png');
...
assert(server.includes("const AGENT_SHARE_BRAND_PREVIEW_VERSION = 'preview-v4'"), 'authentic brand preview must have a stable unlisted version');
...
assert(server.includes("options.previewVersion === AGENT_SHARE_BRAND_PREVIEW_VERSION"), 'authentic brand card must stay behind the unlisted preview version');
assert(server.includes("'/assets/marketing/francis-isabirye-agent-share-v4.png'"), 'authentic brand preview must use the v4 card asset');
...
assert(server.includes("'francis-agent-authentic-brand-preview-v4-20260829'"), 'authentic brand preview must expose a production release marker');
...
assert(fs.existsSync(brandCardPath), 'authentic brand Francis rich-link preview image must exist');
const brandPng = fs.readFileSync(brandCardPath);
assert.strictEqual(brandPng.toString('ascii', 1, 4), 'PNG', 'authentic brand share image must be a PNG');
assert.strictEqual(brandPng.readUInt32BE(16), 1200, 'authentic brand share image must be 1200px wide');
assert.strictEqual(brandPng.readUInt32BE(20), 630, 'authentic brand share image must be 630px high');
