...
test('anonymous public property APIs suppress launch seed QA listings', () => {
  ...
  assert.match(appSource, /const \{ rows: firstPageRows, firstResponse: firstPageResponse \} = await fetchPublicPaginatedRows\(firstPagePath, \{[\s\S]*limit: PUBLIC_LISTINGS_FAST_PAGE_LIMIT,[\s\S]*maxPages: 1,[\s\S]*includeSummary: false/);
  ...
  assert.match(appSource, /const summaryStats = await summaryStatsPromise;\s*const firstPageCategoryTotal = activeCategory \? exactPublicPaginationTotal\(firstPageResponse\) : 0;/);
