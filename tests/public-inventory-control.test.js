...
test('anonymous public property APIs suppress launch seed QA listings', () => {
  ...
  assert.match(appSource, /PUBLIC_LISTINGS_FAST_PAGE_LIMIT = 24/);
  ...
  assert.match(appSource, /applyPublicRows\(firstPageRows, firstPageResponse\);\s*renderAll\(\);/);
  assert.match(appSource, /const \{ rows: publicRows, firstResponse \} = await fetchPublicPaginatedRows\("\/api\/properties\?status=approved&public_only=1", \{[\s\S]*limit: PUBLIC_LISTINGS_BACKGROUND_PAGE_LIMIT,[\s\S]*maxPages: PUBLIC_LISTINGS_BACKGROUND_MAX_PAGES,[\s\S]*includeSummary: false/);
  assert.match(appSource, /await summaryStatsPromise;\s*const featuredRows = await featuredRowsPromise;\s*applyPublicRows\(publicRows, firstResponse, \{ featuredRows, prune: true \}\);\s*renderAll\(\);/);
