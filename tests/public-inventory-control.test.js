...
const publicAgentEligibilityServiceSource = fs.readFileSync('services/publicAgentEligibilityService.js', 'utf8');
...
test('admin live controls use paginated backend snapshots', () => {
  ...
  assert.match(reviewQueueRouteSource, /count_filter: queueType === 'found_online'[\s\S]*'admin_found_online_review_queue'[\s\S]*includeTestLike \? 'admin_active_review_queue' : 'admin_actionable_review_queue'/);
  ...
  assert.match(appSource, /ADMIN_PENDING_QUEUE_RENDER_STEP = 24/);
...
test('anonymous public agent APIs suppress QA broker records', () => {
  assert.match(publicAgentEligibilityServiceSource, /'QA TEST - DELETE'[\s\S]*'SOFT LAUNCH TEST - DELETE'[\s\S]*'TRAINING'[\s\S]*'DEMO'[\s\S]*'SAMPLE'[\s\S]*'PLACEHOLDER'/);
  assert.match(publicAgentEligibilityServiceSource, /function addPublicAgentLaunchTestFilter/);
  assert.match(publicAgentEligibilityServiceSource, /function addPublicAgentSelfRegistrationFilter/);
  assert.match(publicAgentEligibilityServiceSource, /PUBLIC_AGENT_MIN_LIVE_LISTINGS = 2/);
  assert.match(publicAgentEligibilityServiceSource, /function addPublicAgentInventoryFilter/);
  assert.match(publicAgentEligibilityServiceSource, /addPublicAgentLaunchTestFilter\(filters, values, alias\)/);
  assert.match(publicAgentEligibilityServiceSource, /addPublicAgentSelfRegistrationFilter\(filters, alias\)/);
  assert.match(publicAgentEligibilityServiceSource, /addPublicAgentInventoryFilter\(filters, alias\)/);
  assert.match(publicAgentEligibilityServiceSource, /COUNT\(\*\)::int[\s\S]*p\.agent_id = \$\{a\}\.id[\s\S]*p\.status = 'approved'[\s\S]*>= \$\{PUBLIC_AGENT_MIN_LIVE_LISTINGS\}/);
  assert(publicAgentEligibilityServiceSource.includes('${a}.user_id IS NOT NULL'));
  assert(publicAgentEligibilityServiceSource.includes("COALESCE(${a}.email, '') !~* '(qa-test|makaug\\\\.invalid|dummy|sample)'"));
  assert(publicAgentEligibilityServiceSource.includes("COALESCE(${a}.licence_number, '') !~* '^(QA|TEST|DUMMY|SAMPLE)-'"));
  assert(publicAgentEligibilityServiceSource.includes("COALESCE(${a}.licence_number, '') !~* '^(SOCIAL|FOUND-ONLINE|TIKTOK|FACEBOOK|X)-'"));
  assert.match(agentsRouteSource, /addPublicAgentEligibilityFilters\(filters, values\)/);
