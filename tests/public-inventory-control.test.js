...
test('anonymous public property APIs suppress launch seed QA listings', () => {
  ...
  assert.match(appSource, /const \{ rows: publicRows, firstResponse \} = await backgroundRowsPromise;\s*const featuredRows = await featuredRowsPromise;\s*applyPublicRows\(publicRows, firstResponse, \{ featuredRows, prune: true \}\);\s*renderAll\(\);/);
  ...
  assert.match(appSource, /async function fetchPublicCategoryRows\(category, totalCount = 0\)/);
