...
function run() {
  ...
  assert(staffRoutes.includes("process.env.STAFF_SOCIAL_SWEEP_SOURCE_LIMIT || '160'"), 'staff social sweep should allow larger async source batches after the first pool is exhausted');
  assert(staffRoutes.includes("process.env.STAFF_SOCIAL_SWEEP_PAGE_LIMIT || '6'"), 'staff social sweep should allow deeper channel-upload pagination for agency sweeps');
  ...
  assert(app.includes('STAFF_SOURCE_DEEP_SWEEP_MAX_PAGES = 6'), 'staff dashboard should expose a deeper agency-channel sweep profile');
