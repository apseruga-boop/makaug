...
function run() {
  ...
  assert(staffRoutes.includes("router.get('/properties'"), 'staff must have a protected paginated property list route instead of falling back to public APIs');
  ...
  assert(app.includes(') || adminCurrentPendingListings.find('), 'review records opened from later pending-queue pages must remain actionable by exact ID');
  ...
  assert(staffPanelsBody.includes('const [recentActivity, reviewResult]'), 'staff panels endpoint should use one bounded moderation candidate query alongside recent activity');
