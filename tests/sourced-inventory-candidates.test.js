...
test('found-online seed panel hides approved and live records from pending moderation', () => {
  ...
  assert(frontend.includes('fetchAdminPaginatedRows("/api/admin/properties/review-queue?include_total=0", headers, { maxPages: 3 })'), 'dashboard should fetch a bounded protected review-queue snapshot without overloading King dashboard');
...
test('public property images escape and normalize generated SVG evidence cards', () => {
  ...
  assert(frontend.includes('return socialImportListingCardHtml(p, options);'), 'third-party/source-import grid cards should use the native social import tile renderer');
  assert(frontend.includes('data-social-import-tile="1"'), 'social import tiles should carry a stable DOM marker for QA');
  assert(frontend.includes('social-import-source-chip'), 'social import tiles should render a mandatory source chip');
  assert(frontend.includes('social-import-media-badge'), 'social import tiles should render a mandatory media-type badge');
  assert(frontend.includes('socialImportPlatformMeta') && frontend.includes('label: "TikTok"') && frontend.includes('label: "X"') && frontend.includes('label: "Facebook"') && frontend.includes('label: "Instagram"'), 'social import tiles should support TikTok, X, Facebook and Instagram platform chips');
  assert(frontend.includes('socialImportMediaType') && frontend.includes('socialImportDurationLabel') && frontend.includes('socialImportImageCount'), 'social import tiles should distinguish video duration/reel badges from image counts');
  assert(frontend.includes('Imported from ${platform} · reviewed by makaug'), 'approved social import tiles should show reviewed provenance');
  assert(frontend.includes('Imported from ${platform} · Auto-imported'), 'non-approved imported rows should avoid reviewed provenance');
  ...
  assert(frontend.includes('p.cover_image_url') && frontend.includes('extra.cover_image_url'), 'cached cover images should be usable by social import cards');
  ...
  assert(html.includes('king-dashboard-ai-visible-summary-fix-20260710'), 'public app marker should identify the King dashboard visible AI summary fix rollout');
  assert(html.includes('social-import-tiles-20260713'), 'public app marker should identify the social import tile rollout');
  ...
  assert(propertiesRoute.includes('media_type: mediaType'), 'properties API should expose safe social import media type');
  assert(propertiesRoute.includes('media_duration: mediaDuration || null'), 'properties API should expose safe social import media duration');
  assert(propertiesRoute.includes('image_count: imageCount'), 'properties API should expose safe social import image count');
  assert(propertiesRoute.includes('cover_image_url: sourceThumbnailUrl || null'), 'properties API should expose safe cached cover image URL');
...
test('social platform sweeps promote TikTok hashtags, YouTube videos, and X posts to import rows', () => {
  ...
  assert.strictEqual(normalizedYoutube.raw_source_post.youtube_confidence_review.status, 'youtube_api_auto_live_ready');
