CREATE EXTENSION IF NOT EXISTS pgcrypto;

WITH removal_candidates AS (
  SELECT
    p.id,
    p.status AS old_status,
    COALESCE(p.moderation_stage, '') AS old_moderation_stage,
    COALESCE(p.extra_fields->>'source_platform', '') AS source_platform,
    COALESCE(p.extra_fields->>'source_url', p.extra_fields->>'source_post_url', '') AS source_url,
    COALESCE(p.extra_fields->>'source_listing_key', '') AS source_listing_key
  FROM properties p
  WHERE COALESCE(p.status, '') <> 'deleted'
    AND (
      COALESCE(p.source, '') = 'found_online_property_source_v1'
      OR COALESCE(p.listed_via, '') = 'found_online'
      OR COALESCE(p.extra_fields->>'found_online_candidate', '') IN ('true','1','yes')
      OR COALESCE(p.extra_fields->>'found_online', '') IN ('true','1','yes')
      OR COALESCE(p.extra_fields->>'social_search_candidate', '') IN ('true','1','yes')
    )
    AND NOT (
      COALESCE(p.extra_fields->>'preapproved_source_post', '') ~* '^(true|1|yes)$'
      AND COALESCE(p.extra_fields->>'consent_confirmed', '') ~* '^(true|1|yes)$'
      AND COALESCE(p.extra_fields->>'image_rights_confirmed', '') ~* '^(true|1|yes)$'
      AND LOWER(COALESCE(p.extra_fields->>'permission_status', '')) IN (
        'founder_reported_agent_authorised_upload',
        'founder_reported_agent_authorised_listing',
        'founder_confirmed_preapproved',
        'agent_authorised_upload',
        'agent_authorised_listing',
        'agent_preapproved',
        'owner_agent_preapproved'
      )
    )
),
event_rows AS (
  INSERT INTO property_moderation_events (
    property_id,
    actor_id,
    action,
    status_from,
    status_to,
    checklist,
    reason,
    notes,
    delivery
  )
  SELECT
    id,
    'strict_found_online_preapproval_cleanup_20260525',
    'implicit_found_online_approval_removed',
    old_status,
    'deleted',
    jsonb_build_object(
      'explicit_preapproval_required', true,
      'consent_required', true,
      'image_rights_required', true,
      'source_platform', source_platform,
      'source_url', source_url,
      'source_listing_key', source_listing_key
    ),
    'Removed from makaug launch inventory because this found-online row did not carry explicit pre-approval, consent, and image-rights confirmation fields.',
    'Strict launch guardrail: social evidence alone is not enough. Re-import only after the exact social post and media rights are explicitly pre-approved.',
    jsonb_build_object(
      'cleanup_batch', 'strict_found_online_preapproval_20260525',
      'rollback', 'Re-import through source-post import only after pre_approved, consent_confirmed, image_rights_confirmed, and permission_status are documented.'
    )
  FROM removal_candidates
  RETURNING property_id
)
UPDATE properties p
SET
  status = 'deleted',
  moderation_stage = 'removed_missing_explicit_preapproval',
  moderation_notes = CONCAT_WS(
    E'\n',
    NULLIF(p.moderation_notes, ''),
    'Removed 25 May 2026: found-online rows now require explicit source-post pre-approval, consent confirmation, and image-rights confirmation before public publication.'
  ),
  moderation_reason = 'Removed by strict found-online pre-approval cleanup.',
  extra_fields = COALESCE(p.extra_fields, '{}'::jsonb) || jsonb_build_object(
    'removed_from_public_inventory', true,
    'removed_from_public_inventory_at', NOW(),
    'removal_batch', 'strict_found_online_preapproval_20260525',
    'removal_policy', 'explicit_preapproval_required',
    'preapproval_required_for_reimport', true,
    'implicit_approval_blocked', true
  ),
  updated_at = NOW()
FROM removal_candidates rc
WHERE p.id = rc.id;

WITH agent_inventory AS (
  SELECT
    a.id,
    COUNT(p.id) FILTER (WHERE p.status = 'approved')::int AS live_listing_count
  FROM agents a
  LEFT JOIN properties p ON p.agent_id = a.id
  GROUP BY a.id
)
UPDATE agents a
SET
  status = 'suspended',
  verification_reason = CONCAT_WS(
    E'\n',
    NULLIF(a.verification_reason, ''),
    'Hidden 25 May 2026: public broker/source profiles require at least two approved makaug listings after strict found-online pre-approval cleanup.'
  ),
  updated_at = NOW()
FROM agent_inventory ai
WHERE a.id = ai.id
  AND ai.live_listing_count < 2
  AND (
    COALESCE(a.licence_number, '') ~* '^(SOCIAL|FOUND-ONLINE)-'
    OR COALESCE(a.verification_reason, '') ILIKE '%source%'
    OR COALESCE(a.verification_reason, '') ILIKE '%sweep%'
  );
