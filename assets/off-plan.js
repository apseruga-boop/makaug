(function initializeOffPlanFeature() {
  ...
  function localizedUnitLabel(unit = {}) {
    if (offPlanLanguage() === 'en' && clean(unit.label)) return clean(unit.label);
  ...
  function galleryMarkup(project) {
    ...
    const viewAllLabel = escapeHtml(offPlanText('viewPhotos', { count: allImages.length }));
    const preview = `<div class="off-plan-gallery">${images.map((image, index) => `<figure><img src="${escapeHtml(image.url)}" alt="${escapeHtml(imageCaption(project, index))}"><figcaption>${escapeHtml(imageCaption(project, index))}</figcaption>${index === 0 && allImages.length > 3 ? `<button type="button" onclick="openOffPlanGallery()" class="off-plan-gallery-view-all-mobile rounded-lg bg-white/95 text-gray-950 px-3 py-2 text-xs font-black"><i class="fas fa-images mr-1"></i>${viewAllLabel}</button>` : ''}${index === 2 && allImages.length > 3 ? `<button type="button" onclick="openOffPlanGallery()" class="off-plan-gallery-view-all-desktop absolute right-3 top-3 rounded-lg bg-white/95 text-gray-950 px-3 py-2 text-xs font-black"><i class="fas fa-images mr-1"></i>${viewAllLabel}</button>` : ''}</figure>`).join('')}</div>`;
  ...
  async function setOffPlanProjectStatus(id, role, status, publicationMode = '') {
    ...
    const statusMessage = document.querySelector(`[data-off-plan-managed-id="${CSS.escape(id)}"] [data-op-status-message]`);
    ...
    try { await request(`${base}/status`, { method: 'POST', headers: managementHeaders(role), body: { status, publication_mode: publicationMode || null } }); await loadOffPlanManagement(role); }
      ...
      const message = error.payload?.blockers?.join(' ') || error.message;
      if (statusMessage) { statusMessage.textContent = message; statusMessage.classList.remove('hidden'); }
      else alert(message);
    }
  }
  ...
  async function downloadOffPlanBrochure(id, role, slug = 'off-plan-project') {
    try {
      const language = offPlanLanguage();
      const path = `/api/${role === 'admin' ? 'admin' : 'staff'}/off-plan/developments/${encodeURIComponent(id)}/brochure.pdf?lang=${encodeURIComponent(language)}`;
      ...
      const anchor = document.createElement('a'); anchor.href = url; anchor.download = `${slug}-${language}-makaug-review-brochure.pdf`; anchor.click();
