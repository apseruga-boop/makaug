...
async function probeProtectedBackend() {
  ...
  const endpoints = [
    ...
    ['/api/admin/outlook-agent/status', 401],
...
function sourceWiringChecks() {
  ...
  const expectations = [
    ...
    ['auth signup contact OTP request exists', 'routes/auth.js', "router.post('/request-signup-otp'"],
    ['auth signup contact OTP verify exists', 'routes/auth.js', "router.post('/verify-signup-otp'"],
    ['auth verified signup token required before instant account creation', 'routes/auth.js', 'contact_verification_token'],
    ['auth password login returns dashboard redirect payload', 'routes/auth.js', "message: 'Signed in. Opening your makaug.com dashboard.'"],
    ...
    ['admin source registry seed route visible', 'routes/admin.js', "router.post('/property-source-registry/seed'"],
    ['admin source registry list route visible', 'routes/admin.js', "router.get('/property-source-registry'"],
    ['property source registry service exists', 'services/propertySourceRegistryService.js', 'PROPERTY_SOURCE_REGISTRY'],
    ...
    ['frontend public map stale registry cleanup exists', 'index.html', 'function clearStaleMapRegistry()'],
    ['frontend public map DOM ownership check exists', 'index.html', 'function mapInstanceMatchesDom(mapId)'],
    ['frontend public map route recovery exists', 'index.html', 'function refreshActivePublicMapsAfterRoute()'],
    ['frontend public maps force recovery on route change', 'index.html', 'scheduleMapsInit({ force: targetPage !== previousPage })'],
    ['frontend popstate missing fragment recovery exists', 'index.html', 'popstate_missing_fragment'],
    ['frontend Google maps resize after route switch exists', 'index.html', 'google.maps.event.trigger(map, "resize")'],
    ['frontend detail map Leaflet fallback loads explicitly', 'index.html', 'if (!(await ensureLeafletApi())) return;'],
    ...
    ['frontend hero commercial type payload exists', 'index.html', 'setFilter("commercialType", "commercial_type", filters.commercialType)'],
    ['frontend hero land title payload exists', 'index.html', 'setFilter("landTitleType", "land_title_type", filters.landTitleType)'],
    ...
    ['frontend section search shell config exists', 'index.html', 'SECTION_SEARCH_CONFIGS'],
    ['frontend section search route mount exists', 'index.html', 'mountSectionSearchShell(targetPage)'],
    ['frontend section search analytics exists', 'index.html', 'trackEvent("section_search_run"'],
    ['frontend section search backend proof exists', 'index.html', 'recordHeroSearchBackendPayload(buildHeroSearchBackendPayload'],
    ['frontend section search checks sale route', 'index.html', 'backendCategory: "sale"'],
    ['frontend section search checks rent route', 'index.html', 'backendCategory: "rent"'],
    ['frontend section search checks students route', 'index.html', 'backendCategory: "students"'],
    ['frontend section search checks land route', 'index.html', 'backendCategory: "land"'],
    ['frontend section search checks commercial route', 'index.html', 'backendCategory: "commercial"'],
    ['frontend section search checks brokers route', 'index.html', 'backendCategory: "brokers"'],
    ['frontend broker search backend probe uses agents API', 'index.html', 'backend_endpoint: isBrokerSearch ? "agents" : "properties_search"'],
    ['route transition probe checks section search shells', 'scripts/probe-route-transitions.js', 'missing section search shell'],
    ['browser public route probe checks section search shells', 'scripts/probe-public-routes-browser.js', 'SECTION_SEARCH_ROUTES'],
    ['frontend auth details-first create step exists', 'index.html', 'account-access-create-details-step'],
    ['frontend auth verification-before-preferences step exists', 'index.html', 'verifyAccountAccessContactOtp'],
    ['frontend auth preferences-before-password step exists', 'index.html', 'account-access-create-preferences-step'],
    ['frontend auth sign-in forces role dashboard route', 'index.html', 'openSignedInDashboard(dashboardRouteForPortalMode(resolvedUser.portal_mode)'],
    ['frontend auth sign-in full-loads protected dashboard when missing from public shell', 'index.html', 'window.location.href = targetRoute'],
    ['frontend auth inline forgot password reset exists', 'index.html', 'account-access-forgot-wrap'],
    ['frontend property finder dashboard language pack exists', 'index.html', 'FINDER_DASHBOARD_I18N'],
    ['frontend property finder dashboard weather context exists', 'index.html', 'finder-weather-context'],
    ['frontend property finder dashboard owned listings exists', 'index.html', 'finder-owned-listings'],
    ['frontend property finder dashboard owned edit exists', 'index.html', 'openOwnedListingEditor'],
    ['frontend property finder dashboard owned delete exists', 'index.html', 'deleteOwnedListing'],
    ['frontend property need dashboard form exists', 'index.html', 'finder-need-form'],
    ['frontend property need submit exists', 'assets/makaug-app.js', 'submitFinderNeedRequest'],
    ['frontend property need unresolved status exists', 'assets/makaug-app.js', 'needUnresolved'],
    ['frontend safety page stakeholder guidance exists', 'index.html', 'data-safety-stakeholder-grid="1"'],
    ['frontend safety page language wiring exists', 'index.html', 'data-content-i18n="safety.title"'],
    ['frontend account settings edits finder preferences', 'index.html', 'account-pref-goal'],
    ['frontend account settings shows password confirmation', 'index.html', 'account-password-status-msg'],
    ['frontend account settings saves finder onboarding preferences', 'index.html', '/api/property-seeker/onboarding'],
    ['auth profile update accepts safe phone updates', 'routes/auth.js', 'account_phone_changed'],
    ['auth profile update stores profile preferences', 'routes/auth.js', 'profile_data = COALESCE(profile_data'],
    ['property seeker owned listings API exists', 'routes/property-seeker.js', "router.get('/my-listings'"],
    ['property seeker owned listing update API exists', 'routes/property-seeker.js', "router.patch('/my-listings/:id'"],
    ['property seeker owned listing delete API exists', 'routes/property-seeker.js', "router.delete('/my-listings/:id'"],
    ['property seeker owned listing email/phone match exists', 'routes/property-seeker.js', 'ownerMatchSql'],
    ['property seeker owned listing update logging exists', 'routes/property-seeker.js', 'listing_updated_by_owner'],
    ['property seeker owned listing delete logging exists', 'routes/property-seeker.js', 'listing_deleted_by_owner'],
    ['property seeker need request confirmation email log exists', 'routes/property-seeker.js', 'property_need_request_user_confirmation'],
    ['property seeker need request admin alert exists', 'routes/property-seeker.js', 'property_need_request_admin_alert'],
    ['admin property need request list API exists', 'routes/admin.js', "router.get('/property-need-requests'"],
    ['admin property need request status API exists', 'routes/admin.js', "router.patch('/property-need-requests/:id'"],
    ['admin setup property need unresolved count exists', 'routes/admin.js', 'unresolvedPropertyNeedRequests'],
    ['verified finder signup seeds property seeker preferences', 'services/authFlowService.js', 'property_seeker_preferences'],
...
function providerStatusChecks() {
  const groups = [
    ...
    ['email provider', ['SMTP_HOST', 'SMTP_USER', 'SMTP_PASS', 'RESEND_API_KEY', 'MAIL_WEBHOOK_URL', 'MS_GRAPH_CLIENT_ID', 'OUTLOOK_AI_CLIENT_ID']],
    ['Outlook AI Email Agent', ['OUTLOOK_AI_AGENT_ENABLED', 'OUTLOOK_AI_MAILBOX', 'OUTLOOK_AI_CLIENT_ID', 'OUTLOOK_AI_TENANT_ID']],
